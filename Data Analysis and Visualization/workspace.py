...
def calculate_hand_percentages(hand_position, position_counts, total_hands, hands_in_tie):
    ...
    beat_pct = (hands_above / total_hands) * 100
    tie_pct = (hands_tie / total_hands) * 100
    lose_pct = (hands_below / total_hands) * 100
...
def display_hand_rankings(ranked_hands, suits, mode='condensed'):
    ...
    if mode.lower() == 'condensed':
        ...
        print("-" * 110)
        print(f"{'Pos':>4} | {'Hand':^20} | {'Best Hand':<15} | {'Used':<4} | {'Strength':>8} | {'Beat%':>6} | {'Tie%':>6} | {'Lose%':>6}")
        print("-" * 110)
        ...
        for position in sorted_positions:
            ...
            print(f"{position:>4} | {formatted_notations:<20} | {pos_info['hand_type']:<15} | {pos_info['cards_used']:>4} | "
                  f"{pos_info['strength']:>8.2f} | {pos_info['beat_pct']:>6.2f} | {pos_info['tie_pct']:>6.2f} | {pos_info['lose_pct']:>6.2f}")
    
    else:  # detailed mode
        print("\nAll possible two-card combinations ranked (from best to worst):")
        print("-" * 140)
        print(f"{'Pos':>4} | {'Hole Cards':<20} | {'Best Hand':<15} | {'Used':<4} | {'Strength':>8} | {'Beat%':>6} | {'Tie%':>6} | {'Lose%':>6} | {'Best Five Cards'}")
        print("-" * 140)
        ...
        for hand in ranked_hands:
            ...
            print(f"{position_str:>4} | {hole_cards:<20} | {hand_type:<15} | {cards_used:>4} | {strength:>8.2f} | "
...
def calculate_and_display_percentages(filtered_hand, ranked_hands):
    ...
    print(f"Hand Percentages: {beat_pct:.2f}% beat you | {tie_pct:.2f}% tie with you | {lose_pct:.2f}% lose to you")
